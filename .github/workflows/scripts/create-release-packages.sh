...
build_variant() {
  ...
  if [[ -d templates ]]; then
    ...
    # Copy template files to .specify/templates (exclude commands/ and vscode-settings.json)
    find templates -maxdepth 1 -type f -not -name "vscode-settings.json" -exec cp {} "$SPEC_DIR/templates/" \;
